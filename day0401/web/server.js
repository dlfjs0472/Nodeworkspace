...
var server=http.createServer(function(request,response){
    //클라이언트에 응답하기
    //클라이언트에게 보여줄 html 문서를 이루고 있는 코드를 읽어들여서, 응답정보로
    //보내보자!!
    
    //클라이언트가 브라우저의 URL에 어떤 주소를 입력했는지를 조사해보자!!
    //그 조사결과에 따라서 아래 코드 중 어떤코드가 실행될지를 결정짓자
    //request객체란 클라이언트의 요청정보를 가진 객체이기 때문에 클라이언트가 
    ...
    if(url=="/member/form"){
        //회원가입을 희망하는 사람이라면...
        // fs.readFile("읽어들일 파일의 경로", "인코딩방식", function(에러가 뜨면 담을 변수, 파일데이터를 담을 변수){});
        ...
        //쿼리문 수행전에,node.js 가 mysql에 접속을 성공해야한다!!
        var con=mysql.createConnection({
            url:"localhost:3306", //db는 네트워크 프로그램이라 포트 사용함 mysql 포트 :3306
        ...
        sql+=" values('superman','0000','슈퍼맨')"; //한칸 떨어뜨려야 함
