...
var server = http.createServer(function(request, response){
    ...
    */
    ...
    //혹여나, 파라미터가 get방식으로 전송되어 올 경우엔 request.url의 파라미터 까지도 주소로
...
//1:1로 대응되는 요청을 처리한다면... 개발에 효율성 없고
    ...
    request.on("end", function(){
        ...
        con.query(sql, function(err, fields){
            if(err){ //쿼리수행중 심각한 에러 발생
                ...
                //목록요청을 바로 진행!!
                // response.redirect("/member/list"); //클라이언트의 브라우저로 하여금 지정한 url로 다시 요청을 할 것을 명령!!
            }
...
function getDetail(request, response){
    ...
    //url : get방식의 파라미터 추출
    var param=url.parse(request.url, true).query; //true를 매개변수로 넘기면, json형태로 파라미터를 반환해줌

    console.log("상세보기에 필요한 추출 파라미터는 ",url.parse(request.url, true).query);

    var member_id=param.member_id; //json으로 부터 파라미터 변수 추출!!
...
function edit(request, response){
    ...
    //글 수정은 클라이언트로부터 post방식으로 서버에 전송되기 때문에, 그 데이터가
    //body에 들어있다 파라미터를 추출하기 위한 모듈이 querystring이다!!

    //post방식의 데이터는 버퍼에 담겨오기 때문에, 이 따로따로 직렬화되어 분산된 데이터를
    //일단 문자열로 모아서 처리해야 한다
    var content="";
    ...
    request.on("end", function(){
        ...
        var con=mysql.createConnection(conStr);//접속
        /*
        DML(insert,update,delete):매개변수2개
        select:매개변수3개
        */
        con.query(sql, function(err, fields){
            if(err){
...
function del(request, response){
    ...
    // 쿼리문 구성이 끝났으므로, mysql에 접속하여 해당쿼리를 실행하자!!
    var con=mysql.createConnection(conStr);//접속 및 커넥션 객체 반환!
    //커넥션 객체는 접속 정보를 가진 객체이며, 이객체를 통해 쿼리를 수행할 수 있다.. 
    //또한mysql,접속을 해제도 가능하다
    //select문의 경우엔 결과를 가져와야 하기 때문에 가운데 인수,result인수가 필요하지만
    //delete, update, insert 즉 DML은 가져올 레코드가 없기 때문에 인수가 2개면 충문함.
    con.query(sql,function(err,fields){
        if(err){
            console.log(err);
        }else{
